...
// 用来显示用户收集到的题目
public class ActCollectQuestion extends Activity {
	...
	private CollectSqlHelper colSqlHelper;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.act_collect_list);
		MenuTop menu = (MenuTop) findViewById(R.id.collect_list_menutop);
		menu.setTitleText("收藏");
	...
	private void init(){
		mTitle = (TextView) findViewById(R.id.tv_title_collect);
		mOptionA = (TextView) findViewById(R.id.x_a_collect);
		mOptionB = (TextView) findViewById(R.id.x_b_collect);
		mOptionC = (TextView) findViewById(R.id.x_c_collect);
		mOptionD = (TextView) findViewById(R.id.x_d_collect);
		mbtnUp = (Button) findViewById(R.id.up_question_collect);
		mbtnAnswer = (Button) findViewById(R.id.answer_collect);
		mbtnDown = (Button) findViewById(R.id.down_question_collect);

		colSqlHelper = new CollectSqlHelper(getApplicationContext());
	}
	...
	public void showQuestion(){
		...
		String kind = getIntent().getStringExtra("collectquestion_kind");
		ExamQuestiondata = colSqlHelper.getQuestionSqlKind(kind);
		// 2、检查数据库中是否有数据
		if(ExamQuestiondata.size() != 0){
			// 3、把当前的数据展示在界面中
	...
	private void showPopuwindow(int action){
		...
		parent = this.findViewById(R.id.collect_main);
	}

	// 删除收藏
	private void deleteCollectQuestion(){
		...
		System.out.println("修改前的postion="+questionpostion);
		if(questionpostion > 0){
			questionpostion = questionpostion-1;
		}
		if(ExamQuestiondata.size() != 0){
		...
		colSqlHelper.DeleteCollectQuestionSql(eqb);
	}
